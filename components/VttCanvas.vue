<template>
  ...
  <!-- <v-container fluid class="pa-0 fill-height bg-grey-darken-4"> -->
    ...
    <v-card class="position-absolute ma-4 pa-3 token-hud" elevation="5" min-width="250">
      ...
      <v-layer>
        ...
        >
          ...
          <v-circle 
            ...
            :config="{ radius: TOKEN_RADIUS + 8, stroke: '#00E676', strokeWidth: 3, dash: , opacity: 0.8 }" 
          />
...
const tokenId = useCurrentUser().value.uid

const mapData = useDocument(doc(useFirestore(),'campaigns',useRoute().params.id,'maps',mapId))
...
const tokens = useCollection(collection(useFirestore(),'campaigns',useRoute().params.id,'maps',mapId,'tokens'))

const currentUserId = ref(useCurrentUser().value.uid)
...
const selectedTokenStats = ref({
  maxMovement: players[useCurrentUser().value.uid].movement*12, // Max feet allowance per standard action turn
...
const remainingMovement = ref(players[useCurrentUser().value.uid].movement*12)
...
//   try{
//     const tokenDoc = doc(useFirestore(), 'campaigns', useRoute().params.id , 'tokens',tokenId)
...
const handleTokenDragEnd = async (event) => {
  ...
  await updateDoc(doc(useFirestore(),'campaigns',useRoute().params.id, 'maps', mapId, 'tokens', tokenId), { x: snappedX, y: snappedY });
...
const spawnNewToken = async () => {
  const activeProfile = playerProfiles.find(p=>p.id==currentUserId.value)
  try{
    const tokenSubcollection = collection(useFirestore(), 'campaigns', useRoute().params.id , 'maps', mapId, 'tokens')
    await addDoc(tokenSubcollection, {
      name:activeProfile.name,
      ownerId:currentUserId.value, //google says use activeProfile.id here
